...
预热总结轮不交给模型自由生成，也不作为用户可见回复。第 5 轮预热结束后，后端根据已持久化的 `warmup_result` 生成后台分析，然后立即进入第一个未覆盖计划话题继续自然提问。
...
预热总结是产品状态机输出，不依赖模型思维链。该轮只写入后台监控，不直接展示给用户；用户侧会看到围绕第一个计划话题的自然追问。诊断边界和风险提示由 `safety_notice` 单独展示。
...
- 明显自伤/自杀关键词会触发 high；
- 预热阶段能形成并持久保存本轮计划话题；
- 预热结束总结由后端固定结构生成并进入后台，不调用模型自由生成，也不直接发给用户；
