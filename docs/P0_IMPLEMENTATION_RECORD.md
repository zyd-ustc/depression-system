...
- 后端根据已持久化的 `warmup_result` 直接生成固定结构回复，即使 DeepSeek 可用也不调用模型；
- 输出用户可读的拟覆盖话题、患者初步信息和症状判断；
- 诊断边界和风险提示由 `safety_notice` 单独展示；
- 不依赖模型思维链或自由生成，避免出现“我来整理一下”一类过渡话术；
- 不允许继续预热提问。
...
- 状态面板单独展示语气 skill、知识查询状态、知识来源和安全提示；
