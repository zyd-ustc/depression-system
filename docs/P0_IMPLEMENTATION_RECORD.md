...
- 模型必须输出用户可读的拟覆盖话题、患者初步信息、症状判断和风险边界；
- 不允许继续预热提问。
...
- 路由：`/monitor`；
- 接口：`GET /api/monitor/current`；
- 当前复用登录用户 token，不引入单独管理员权限；
- 展示预热进度、预热生成的话题列表、患者初步信息表、对话历史、患者当前状态；
- 页面每 4 秒轮询一次，随着对话更新。
...
- `/monitor` 目前是同一登录用户的监控视图，尚未做正式后台账号、权限分级和多患者列表。
