...
- `warmup_turns`：预热轮数；
- `warmup_completed`：预热是否已经完成；
- `warmup_result`：5 轮预热后的结构化结果，包括话题列表、患者初步信息和症状判断；
...
- 预热阶段固定最多 5 轮；
- 第 5 个用户轮次后强制结束预热，不再继续预热提问；
- 预热结束时生成并持久化 `warmup_result`；
- `warmup_result` 包括后续话题列表、患者初步信息、功能影响、支持线索、症状判断、风险边界；
...
同一会话一旦进入 `ended`，后续普通输入会保持 `already_ended`，只做简短确认，不再重新开启话题探索。若后续内容出现高风险关键词，仍优先进入安全支持。
...
风险与线下求助边界不再揉进结束回复正文，由 `safety_notice` 单独展示。
...
- `src/product_app/tone_skill.py`
- `src/product_app/safety_notice.py`
...
- 主回复、风险/边界提示、RAG 查询状态和语气 skill 状态已经拆成独立字段；
- `assistant_reply` 只负责对话语气，不再混入免责声明、线下求助边界或 RAG 来源说明；
- `safety_notice` 由后端按风险等级生成，普通回复不负责承载警告；
...
- 该话题只作为后台状态转场概念，不再作为用户可见回复输出；
- 第 5 个用户轮次后，后端生成并持久化 `warmup_result`，然后立即选择第一个未覆盖计划话题继续自然提问；
- 患者初步信息、症状判断、拟覆盖话题和技术状态只在管理员后台查看；
- 用户侧不会看到“预热已完成。以下是……”这类内部工作框架。
...
- 用户侧状态面板只展示用户、阶段、会话、下一步和必要安全提示；
- 管理员后台展示风险、分数、话题计划、已覆盖、症状判断、模型、语气 skill、RAG 查询状态和知识来源；
- 隐私同意只作为必要流程，不在页面中过度强调。

后台监控页面：

- 路由：`/monitor`；
- 管理员接口：`GET /api/admin/monitor`；
- 普通用户继续最近会话接口：`GET /api/conversations/latest`；
- 管理员账号由 `ADMIN_USERNAME` 和 `ADMIN_PASSWORD` 预先配置；
- 普通用户没有后台入口，访问管理员监控接口会返回 403；
- 管理员可以查看所有用户的咨询会话，选择某个用户/会话后查看预热进度、患者初步信息表、对话历史、患者当前状态；
- 页面每 4 秒轮询一次，随着对话更新。
...
- 后台监控已做管理员账号隔离，但仍是 P0 演示级权限模型，生产环境需要接入正式身份认证、审计日志和外部数据库。
