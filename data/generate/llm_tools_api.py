...
def qwen_client_init():
    # 兼容 DeepSeek/OpenAI 兼容接口的场景：通过环境变量注入
    # export DEEPSEEK_API_KEY="..."
    # export DEEPSEEK_BASE_URL="https://api.deepseek.com/v1"
    openai_api_key = os.getenv("DEEPSEEK_API_KEY", None) or os.getenv("OPENAI_API_KEY", None) or ""
    if not openai_api_key:
        raise RuntimeError(
...
def api_load_for_extraction(model_name, input_sentence):    #extract kv pair
    ...
    prompt = '提取文本中所有形如A：B的键值对，以json格式输出，不允许输出其他文字！'
...
def api_patient_experience_trigger(model_name, dialogue_history, path):    #返回生成的背景故事
    ...
    response = chat_response.choices[0].message.content
    if 'True' in response and path and os.path.exists(path):
