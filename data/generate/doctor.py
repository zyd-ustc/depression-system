...
class Doctor(llm_tools_api.DoctorCost):
    ...
    def _cards_init_persona(self):
        ...
        self.doctor_persona = (
            "你是一名精神科/心理科医生（CBT取向），正在进行一次门诊问诊与初步干预。"
            "目标：完成抑郁相关困扰的结构化评估（症状、病程、功能损害、诱因、既往史、物质/药物、睡眠食欲、风险评估），"
            "并在合适时识别患者可能的认知偏差，做一次非常简短的CBT式干预（证据检验/替代想法/一个小行动）。"
            "要求：每轮只问1个核心问题或给1个小指令；口语化、非说教；必要时用一句话共情；不要输出空行。"
            "安全：若涉及自伤/自杀，只做风险评估与求助建议，不要出现任何方式/步骤/工具细节。"
        )
        self.patient_persona = (
            f"患者档案：{age}岁，{gender_cn}，职业/状态：{occ}，MBTI：{mbti}。"
            f"（已知其更常见的思维模式倾向：{bias}；你不要直接把偏差名称‘扣帽子’，而是用验证/探索的方式带出来。）"
        )
        self.messages = [{"role": "system", "content": self.doctor_persona}]

    def doctor_response_gen_cards(self, dialogue_history) -> Tuple[str, bool, float]:
        if not self.use_api:
        ...
        if self.dialbegin:
            user_prompt = (
                self.patient_persona
                + "\n现在开始问诊。先用开放式问题让患者讲“最近最困扰的一件具体事”，不要直接抛量表名。"
                + "\n输出你对患者说的一整段话（不要空行）。"
            )
            ...
            doctor_text = chat_response.choices[0].message.content
            self.messages.pop()
            self.dialbegin = False
            return doctor_text, False, super().get_cost()

        user_prompt = (
            self.patient_persona
            + "\n你与患者的对话历史如下："
            + str(history)
            + "\n请基于对话历史继续推进抑郁症问诊与初步干预："
            + "\n- 你需要逐步覆盖：核心症状(情绪低落/兴趣减退/精力)、持续时间、睡眠食欲、注意力、无价值感/内疚/自责、焦虑/躯体化、功能损害、诱因、既往史/家族史、物质/药物、风险评估(消极意念/计划/意图/保护因素)。"
            + "\n- 当信息足够时，输出以“诊断结束，你的诊断结果为：”开头的一段总结，包含：初步诊断(可写‘抑郁发作/抑郁状态/抑郁障碍疑似’+严重程度推测)、风险评估、建议(就医/复诊/心理治疗CBT/生活方式/危机求助)。"
            + "\n- 若尚未结束：每轮只问1个核心问题或给1个小指令；口语化；不要空行。"
        )
        self.messages.append({"role": "user", "content": user_prompt})
        chat_response = self.client.chat.completions.create(
            model=self.model_name,
        ...
        doctor_text = chat_response.choices[0].message.content
        self.messages.pop()
        is_end = isinstance(doctor_text, str) and ("诊断结束，你的诊断结果为" in doctor_text)
        return doctor_text, is_end, super().get_cost()
